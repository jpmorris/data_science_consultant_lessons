...
<!-- STAGE 2/3 (nexttoken-stage-2-cot): reasoning -- <think> block generates
     word-by-word on the raw-context pane, terminal only ever shows a
     "Thinking..." placeholder, never the reasoning content itself -->
...
usually hidden from you. The `<think>` tag is DeepSeek R1's actual,
publicly-documented output format — say that explicitly if asked, since
other labs (Anthropic, OpenAI, Google) handle this differently, mostly
hiding raw reasoning entirely rather than using a visible tag. "Ask it to
think step by step" describes *prompted* CoT (the 2022-era technique); by
2026 frontier models increasingly do this natively via RL training, not
because anyone asked — worth saying out loud if the audience seems to
expect the older framing.
:::
