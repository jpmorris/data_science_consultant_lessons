...
models have been trained at the scale of GPT-4"), Stanford HAI AI Index
2025. Chart above is illustrative only &mdash; the real gap between these
...
## Word Embeddings, Visually {.heading-accent}

:::: {.columns}

::: {.column width="62%"}
![](images/word-embedding-illustration.svg){fig-align="center" height="600"}
:::

::: {.column width="38%"}
::: {.callout-panel}
Word embeddings pack meaning into geometry — similar words end up near each
other, and some relationships become directions you can do arithmetic on.
:::
:::
...
::: {.notes}
Image: "Word embedding illustration," Wikimedia Commons, CC BY-SA
4.0. Caveat to say out loud: the famous "king − man + woman ≈ queen" example is
real but oversold — it only holds cleanly with certain normalization tricks, not
as a raw, unqualified fact. Say that explicitly if asked.
:::

## The Transformer Still Rules {.heading-accent}

![Encoder-decoder Transformer architecture, from "Attention Is All You Need" (Vaswani et al., 2017).](images/transformer-architecture.png){fig-align="center"
height="700"}

::: {.notes}
Image: Wikimedia Commons, CC BY-SA 4.0. Attention: every token can
directly attend to every other token, in parallel — no recurrence needed.
Nothing has dethroned this as the dominant architecture since 2017, but there
are real contenders:
...
- Pair programming / knowledge transfer for handoff
